...
const getRekapPenilaianData = async (startDate, endDate, name) => {
    let query = `
        SELECT 
            ...
            SUM(CASE WHEN rp.status IN ('Tepat Waktu', 'Tepat Waktu & PSW', 'Terlambat Toleransi', 'Terlambat I', 'Terlambat II') THEN 1 ELSE 0 END) AS total_hadir,
...
const getRekapBulananData = async (bulan, tahun, name) => {
    ...
    let query = `
        SELECT
            ...
            SUM(CASE WHEN rp.status IN ('Tepat Waktu', 'Tepat Waktu & PSW', 'Terlambat Toleransi', 'Terlambat I', 'Terlambat II')
